...
Since Sensu already uses [eventmachine](https://github.com/eventmachine/eventmachine), you just have to ensure that em-http-request gem is present inside Sensu's embedded Ruby. Also Sensu 0.25.x doesn't bundle with multi_json:
* `em-http-request`
* `multi_json`

By default, this extension uses in memory metrics caching : it batches InfluxDB writes. The cache is flushed every __500 items__ or __6 seconds__ (this values can be changed in conf).
...
This is an extension that runs in the sensu-server runtime which makes it more performant. Additionally it's using batch submission to InfluxDB by writing all the points for a given series at once.
...
Additionally I'd like to leave open the possibility to add tags per client but not until there is a good use case as the possibility for incorrect use easily exists.

It could work the same way as check options.
