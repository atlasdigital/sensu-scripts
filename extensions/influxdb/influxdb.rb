# Much of this work was done by https://github.com/BarthV/sensu-influxdb-extension
#
    ...
    class InfluxDB < Handler
      ...
      def run(event_data)
        ...
        event[:check][:output].split(/\r\n/).each do |line|
          ...
          output = [key, values, time.to_i].join(' ')
          @buffer[event[:check][:influxdb][:database]][event[:check][:time_precision]].push(output)
